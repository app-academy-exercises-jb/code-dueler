const finishGame = (pubsub, game) => pubsub.publish("gameEvent", {
  ...game,
  status: "over",
});

const endGame = pubsub => (input, player) => setTimeout(() => {
  const game = { p1, p2, spectators, gameId } = input;

  if (pubsub.subscribers[p1._id] !== undefined && pubsub.subscribers[p2._id] !== undefined) {
